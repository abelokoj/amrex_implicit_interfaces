...
cd LSA_Rayleigh_Python && chmod +x scripts/*.sh && python3 -m pytest -q
```
...
That last point is the most common cause of a job that fails immediately after waiting hours in queue. A solver built against one MPI implementation and launched under another does not fail at link time; it fails at run time, on the compute node, after the allocation has been granted. The submit scripts load `gnu/13` and `openmpi`, together with `python`; if `module list` on the build node reports something else, edit them to match before submitting anything.

The Python stack is installed once with `pip install -r requirements.txt` in `LSA_Rayleigh_Python`, and each job checks that `numpy`, `scipy` and `yt` import before doing any work, which costs seconds against the loss of a whole task later. Where installing `yt` on the cluster proves awkward, the Fortran implementation performs the same analysis with no Python dependency at all and is the better choice there.
...
python3 python/run_lsa.py dispersion --root runs --kr0 0.2,0.3,0.5,0.7,0.9
```
