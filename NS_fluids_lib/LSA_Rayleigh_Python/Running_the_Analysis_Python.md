...
| this directory | `$AII/NS_fluids_lib/LSA_Rayleigh_Python` | the analysis code, and all of its output |
...
No program used here resides outside `$AII`. The analysis programs are the scripts in `python/`, invoked as `python3 python/<name>.py` from this directory; nothing is installed onto the system path, and no copy of them is expected anywhere outside the solver tree.
...
The Python stack is installed separately, from this directory:

```bash
pip install -r requirements.txt        # numpy, scipy, matplotlib, yt, pytest
```

Only `numpy` and `scipy` are required for the dispersion relation and the decomposition. Reading AMReX plotfiles requires `yt`, without which no calculation can be post-processed here; `matplotlib` is required only for the figures of Step 9. Where installing this stack on a cluster is unwelcome, the Fortran implementation performs the same analysis with no Python dependency at all.
...
## 5. Test the analysis tools

Issued from this directory. Nothing is compiled here, but the test suite reads `inputs/inputs_rayleigh_template.txt` by a relative path and must therefore be run from here rather than from `scripts/`.

```bash
cd $AII/NS_fluids_lib/LSA_Rayleigh_Python
chmod +x scripts/*.sh
python3 -m pytest -q
```
...
The suite reports 74 passing assertions when `yt` is present. Without `yt` the end-to-end plotfile test is skipped and 62 pass, which establishes the dispersion relation, the decomposition and the proportionality test, but not the plotfile path on which every measurement depends. The suite must pass before any calculation is attempted.
...
```bash
mkdir -p $AII/NS_fluids_lib/LSA_Rayleigh_Python/runs/k0.7_eps0.02
cd $AII/NS_fluids_lib/LSA_Rayleigh_Python/runs/k0.7_eps0.02
...
The twin is advanced identically, from `$AII/NS_fluids_lib/LSA_Rayleigh_Python/runs/k0.7_base` with the deck `inputs.k0.7.base`, and on the same number of ranks. Altering the domain decomposition between the two perturbs round-off-level behaviour, which is the scale at which the measurement is made.
...
```bash
cd $AII/NS_fluids_lib/LSA_Rayleigh_Python
python3 python/dump_amplitude.py runs/k0.7_eps0.02 8.976 \
    runs/records/k0.7.dat
...
```bash
cd $AII/NS_fluids_lib/LSA_Rayleigh_Python
python3 python/make_inputs.py sweep-eps --kr0 0.7 \
    --eps 5e-3,1e-2,2e-2,4e-2 \
...
The amplitude curve gives no warning of this: on a logarithmic axis it looks like a clean exponential long before it is one. The local growth rate does give warning, by still rising at the end of the record. That check is now performed automatically by `python/transient.py` for every measurement, and it prints the residual transient, the extrapolated asymptote and the run length required. A rate should not be quoted when it reports that the run is too short.


## 9. Figures

Issued from this directory. Figures follow the house style defined in `python/pub_style.py`: Computer Modern typography, inward ticks on all four sides, no axis padding, a data line width of 1.0 point, and PNG, PDF and SVG output at 600 dpi. Nothing in the figure scripts restates those settings, so a change to the house style takes effect everywhere at once.
...
```bash
cd $AII/NS_fluids_lib/LSA_Rayleigh_Python
python3 python/dump_amplitude.py runs/k0.7_eps0.02 8.976 runs/bundles/k0.7/amplitude.dat
python3 python/dump_profiles.py runs/k0.7_eps0.02 8.976 runs/bundles/k0.7 --field y_velocity

# all four figures, from the bundle alone; no yt, no plotfiles
python3 python/plot_results.py --bundle runs/bundles/k0.7 --kr0 0.7 --outdir figures
```
...
```bash
python3 python/plot_results.py --rundir runs/k0.7_eps0.02 --wavelength 8.976 --kr0 0.7 --field y_velocity --outdir figures
```
...
```bash
python3 python/make_report_figures.py --datadir validation_data --archive validation_data/validation_kr0_0.7.json --outdir figures
```
...
```bash
python3 python/fit_records.py                    # every record, fitted and tabulated
python3 python/fit_records.py --t-min 4.0        # an explicit fit window
python3 python/plot_dispersion_sweep.py          # the measured dispersion figure
```
...
python3 python/plot_dmd.py --perturbed runs/k0.7_eps0.02 --base runs/k0.7_base --outdir figures
python3 python/plot_dmd.py --replot --outdir figures        # redraw without decomposing again

# the extracted eigenvector against the analytic eigenfunction
python3 python/plot_mode_shape.py --perturbed runs/k0.7_eps0.02 --base runs/k0.7_base --outdir figures

# growth rate over the wavenumber and Ohnesorge plane, surface and contour
python3 python/plot_ohnesorge.py --measured 0.7 0.02 --outdir figures

# break-up, from a run carried past the linear regime to pinch-off
python3 python/plot_pinchoff.py --bundle runs/bundles/pinch32 --kr0 0.7 --cells-per-r0 32 --outdir figures

# cost of the Jacobian-free path, with the machine recorded alongside
python3 python/benchmark_jvp.py --perturbed runs/k0.7_eps0.02 --base runs/k0.7_base --outdir figures

# the twin-run architecture, drawn rather than measured
python3 python/plot_schematic.py --outdir figures

# machine and run provenance on its own
python3 python/machine_info.py $AII
```
