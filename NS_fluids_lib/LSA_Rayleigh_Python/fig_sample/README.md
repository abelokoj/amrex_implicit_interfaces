...
```bash
python3 python/make_report_figures.py --datadir validation_data --archive validation_data/validation_kr0_0.7.json --outdir fig_sample
python3 python/plot_results.py --json validation_data/validation_kr0_0.7.json --kr0 0.7 --t-fit 4.5 --outdir fig_sample
```

in the Python tree, or the same two commands with `plotting/` in place of `python/` in the Fortran tree. Both draw from stored records rather than from plotfiles, so neither needs `yt` and neither needs the run directories.
