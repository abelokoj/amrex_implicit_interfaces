...
# Checked once, here, before any task is submitted. Every task of the array shares this working directory, so installing from inside a task would have siblings interfering with one another. That failure is quiet: the task exits, the sweep runner logs the case as failed, and sacct reports the task COMPLETED.
if ! python3 -c 'import numpy, scipy, yt' 2>/dev/null; then
  echo "error: numpy, scipy and yt must import; run 'pip install -r requirements.txt' first. Nothing submitted." >&2
  exit 1
fi
