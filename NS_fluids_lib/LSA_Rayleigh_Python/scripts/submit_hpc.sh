...
#
# Submit from amrex_implicit_interfaces/NS_fluids_lib/LSA_Rayleigh_Python:
#
...
# The job begins in the directory from which it was submitted, which the instructions above fix as LSA_Rayleigh_Python. Every path below is relative to it, and every file the job writes stays inside it.
mkdir -p runs
...
# Confirm the Python stack is present and working on the compute node rather than assuming that a login-node environment carries over. A failure here costs seconds; a failure after the sweep has run costs the sweep.
module load python
python3 -m pytest -q
