...
# ROOT is LSA_Rayleigh_Fortran; NSDIR is NS_fluids_lib, which holds both the
...
if [[ ! -x "$BIN/make_inputs" || ! -x "$BIN/run_lsa" ]]; then
...
if (( NPROCS > 1 )); then
  ...
  if (( probe > 0 )); then
    ...
    echo "Re-run with 1 rank, or repair the launcher; see Running_the_Analysis_Fortran.md, Section 11." >&2
    exit 1
  fi
fi
...
( cd "$ROOT" && "$BIN/make_inputs" single --kr0 "$KR0" --eps "$EPS" \
    --cells-per-r0 "$CELLS" --rmax 4.0 --n-periods 5.0 --snapshots 120 \
    --outdir "$DECKS" )

# make_inputs writes compact decimal tags, e.g. eps 2e-2 -> "0.02".
EPS_TAG="$(printf '%s' "$EPS" | awk '{printf "%.10f", $1}' \
           | sed -e 's/0*$//' -e 's/\.$//')"
KR0_TAG="$(printf '%s' "$KR0" | awk '{printf "%.10f", $1}' \
           | sed -e 's/0*$//' -e 's/\.$//')"
...
"$BIN/dump_amplitude" "$PERT_DIR" "$LAMBDA" "$BUNDLE/amplitude.dat"
"$BIN/dump_profiles" "$PERT_DIR" "$LAMBDA" "$BUNDLE" y_velocity

echo
echo "Draw the figures with:"
echo "  python3 plotting/plot_results.py --bundle $BUNDLE --kr0 $KR0"
echo
echo "Done. Before this number is quoted, run the proportionality test:"
echo "  see Running_the_Analysis_Fortran.md, Step 7."
