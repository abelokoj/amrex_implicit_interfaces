...
if [[ ! -x ./bin/make_inputs || ! -x ./bin/dump_amplitude || ! -x ./bin/dump_profiles ]]; then
  log "building the analysis tools"
  make >/dev/null || { log "ERROR: make failed"; exit 1; }
fi
...
if (( NPROCS > 1 )); then
  if [[ "$LAUNCHER" == mpirun* ]] && \
    ...
    log "ERROR: mpirun cannot reach a process manager and would start $NPROCS independent single-rank copies of the solver, all writing into the same directory. Re-run with 1 rank or repair the launcher; see Running_the_Analysis_Fortran.md, Section 11."
    exit 1
  fi
fi
...
while read -r label kr0 eps cells rmax nper snaps rest; do
  ...
  if ! ./bin/make_inputs single --kr0 "$kr0" --eps "$eps_use" \
  ...
  # make_inputs names decks by wavenumber and amplitude; select the twin deck
  # for a .base label and the perturbed deck otherwise.
  ktag=$(printf '%s' "$kr0" | awk '{printf "%.10f", $1}' \
         | sed -e 's/0*$//' -e 's/\.$//')
  case "$label" in
    *.base) deck="$DECKDIR/inputs.k${ktag}.base" ;;
    *)      etag=$(printf '%s' "$eps_use" | awk '{printf "%.10f", $1}' \
                   | sed -e 's/0*$//' -e 's/\.$//')
  ...
  rows=0
  if ./bin/dump_amplitude "$RUNROOT/$label" "$lambda" "$record" >/dev/null 2>&1
  then
  ...
  if (( rows >= 2 )); then
    ...
    if ! ./bin/dump_profiles "$RUNROOT/$label" "$lambda" "$casebundle" ${FIELDS%% *} >/dev/null 2>&1; then
